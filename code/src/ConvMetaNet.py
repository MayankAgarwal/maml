...
class ConvMetaNet(ConvNet):
	...
	def forward(self, task):
		...
		tr_pre_loss, tr_pre_acc = evaluate(self, train_loader, dtype=self.dtype, long_dtype=self.long_dtype)
		val_pre_loss, val_pre_acc = evaluate(self, val_loader, dtype=self.dtype, long_dtype=self.long_dtype)
		
		metanet_weights = OrderedDict((name, param) for (name, param) in self.named_parameters())

		print "Meta-net update step: ",
		...
		tr_post_loss, tr_post_acc = evaluate(self, train_loader, metanet_weights, dtype=self.dtype, long_dtype=self.long_dtype)
		val_post_loss, val_post_acc = evaluate(self, val_loader, metanet_weights, dtype=self.dtype, long_dtype=self.long_dtype)
